document.addEventListener('DOMContentLoaded', function() {
  ...
  function initializeSlider(sliderId, valueDisplayId, calculationCallback, isCurrency = false) {
    ...
    slider.oninput = function() {
      ...
      calculationCallback(); // Trigger calculation on slider move
  ...
  function updateCurrentFields() {
    ...
    var updatedPatientNum = patientNum + sliderValue;
    var perPx = document.getElementById('perPx').value || '0';
    var conversion = document.getElementById('conversion').value || '0';
    var avConsultation = document.getElementById('avConsultation').value || '0';
    var multiPairs = document.getElementById('multiPairs').value || '0';
  
    // Update "current" fields in each row
    var rows = document.querySelectorAll('.row .item-2');
    if (rows.length >= 5) {
        rows[1].textContent = updatedPatientNum ;
        rows[2].textContent = perPx;
        rows[3].textContent = conversion;
        rows[4].textContent = avConsultation;
        rows[5].textContent = multiPairs;
    }
  }
